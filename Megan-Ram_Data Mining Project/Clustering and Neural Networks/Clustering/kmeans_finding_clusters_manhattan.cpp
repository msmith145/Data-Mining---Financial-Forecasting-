...
ld computedist(int cindex, int iindex){
	...
	for(int i =0 ; i<input_cols; i++){
		...
		if(d > maxi )continue;
		d += absolute(center[cindex][i] - inputdata[iindex][i]);
	}
	return d;	
}
ld euclidean_computedist(int cindex, int iindex){
	ld d = 0;
	for(int i =0 ; i<input_cols; i++){
		if(center[cindex][i] - inputdata[iindex][i] < eps) continue;
		if(d > maxi )continue;
...
ld intracluster(){
	ld sum  = 0;
	 for(int i = 0; i <input_rows; i++){
	 	...
	 	sum += euclidean_computedist(nearest_center[i], i)*euclidean_computedist(nearest_center[i], i);
